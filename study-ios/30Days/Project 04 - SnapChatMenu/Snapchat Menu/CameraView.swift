...
//            let backCamera = AVCaptureDevice.devices(for: .video).first!
        ...
        if #available(iOS 10.0, *) {
            let bCamera = AVCaptureDevice.DiscoverySession(deviceTypes: [.builtInWideAngleCamera], mediaType: .video, position: .front).devices
            backCamera = bCamera.first
        } else {
             //Fallback on earlier versions
            let devices = AVCaptureDevice.devices(for: .video)
            guard devices.count > 0 else { return } /// 初始化摄像头设备
            guard let Camera = devices.filter({  return $0.position == .front }).first else { return }
            //backCamera = AVCaptureDevice.devices(for: AVMediaType.video).first
            backCamera = Camera
        }
        // 已经不能再使用了
