...
class TinyPixView: UIView {
    ...
    */
    ...
    private func drawBlockAt(row: Int, column: Int) {
        ...
        let color = document.stateAt(row: row, column: column) ? UIColor.black : UIColor.white
    ...
    private func toggleSelectedBlock() {
        if selectedBlockIndex.row != -1 && selectedBlockIndex.column != -1 {
            ...
            document.undoManager?.prepare(withInvocationTarget: document.toggleStateAt(row: selectedBlockIndex.row, column: selectedBlockIndex.column))
            setNeedsDisplay()
        }
    }
